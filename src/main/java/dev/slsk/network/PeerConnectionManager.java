...
public final class PeerConnectionManager implements IPeerConnectionManager {
    ...
    private final ConcurrentHashMap<String, CompletableFuture<IMessageConnection>> messageConnections =
    ...
    public PeerConnectionManager(PeerConnectionManagerClient client) {
    ...
    public List<PeerEndpoint> getMessageConnections() {
        ...
        for (CompletableFuture<IMessageConnection> future : messageConnections.values()) {
            IMessageConnection connection = future.join();
    ...
    public CompletableFuture<IMessageConnection> getCachedMessageConnectionAsync(String username) {
        CompletableFuture<IMessageConnection> cached = messageConnections.get(username);
        if (cached == null) {
    ...
    private CompletableFuture<IMessageConnection> establishIncomingMessageConnection(
        ...
        diagnostic.debug("Inbound message connection to " + username + " ("
                ...
                + ") accepted. (type: " + incomingConnection.getType()
                + ", id: " + incomingConnection.getId() + ")");
        IMessageConnection connection = connectionFactory.getMessageConnection(
                username,
                incomingConnection.getIpEndPoint(),
    ...
    private CompletableFuture<IMessageConnection> establishInboundIndirectMessageConnection(
    ...
    private CompletableFuture<IMessageConnection> establishRacingMessageConnection(
            String username, InetSocketAddress ipEndPoint, int solicitationToken, CancellationToken cancellationToken) {
        LinkedCancellation directCancellation = new LinkedCancellation(cancellationToken);
        LinkedCancellation indirectCancellation = new LinkedCancellation(cancellationToken);
    ...
    private CompletableFuture<IMessageConnection> getMessageConnectionOutboundDirectAsync(
            String username, InetSocketAddress ipEndPoint, CancellationToken cancellationToken) {
    ...
    private CompletableFuture<IMessageConnection> getMessageConnectionOutboundIndirectAsync(
    ...
    private void messageConnectionDisconnected(IConnection sender, ConnectionDisconnectedEventArgs eventArgs) {
        ...
        if (messageConnections.remove(connection.getUsername(), CompletableFuture.completedFuture(connection))) {
            ...
            CompletableFuture<IMessageConnection> cached = messageConnections.get(connection.getUsername());
            if (cached != null
    ...
    private static final class LinkedCancellation implements AutoCloseable {
        ...
        private LinkedCancellation(CancellationToken parent) {
            registration = PeerConnectionManager.token(parent).register(source::cancel);
        }
