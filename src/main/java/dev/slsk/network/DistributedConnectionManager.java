...
public final class DistributedConnectionManager implements IDistributedConnectionManager {
    ...
    private final ConcurrentHashMap<String, CompletableFuture<IMessageConnection>> childConnections =
    ...
    private volatile IMessageConnection parentConnection;

    /** Creates a manager with default collaborators. */
    public DistributedConnectionManager(DistributedConnectionManagerClient client) {
    ...
    public boolean hasParent() {
        IMessageConnection current = parentConnection;
    ...
    public CompletableFuture<Void> addOrUpdateChildConnectionAsync(String username, IConnection incomingConnection) {
        ...
        CompletableFuture<IMessageConnection> future = childConnections.compute(username, (key, cached) -> {
    ...
    public CompletableFuture<Void> getOrAddChildConnectionAsync(ConnectToPeerResponse response) {
        ...
        CompletableFuture<IMessageConnection> future = childConnections.computeIfAbsent(response.getUsername(), key -> {
            cached.set(false);
        ...
        return future.handle((connection, failure) -> {
            ...
            if (!(cause instanceof CancellationException)) {
                diagnostic.debug("Purging child connection cache of failed connection to "
                        ...
                        + response.getIpEndPoint() + ").");
                CompletableFuture<IMessageConnection> removedRecord = childConnections.remove(response.getUsername());
    ...
    public void removeAndDisposeAll() {
        ...
        IMessageConnection parent = parentConnection;
        if (parent != null) {
            parent.close();
        }
    ...
    void handleParentCandidateMessage(IMessageConnection connection, MessageEventArgs eventArgs) {
        try {
    ...
    private CompletableFuture<IMessageConnection> establishDirectChild(
        ...
        IMessageConnection connection = connectionFactory.getDistributedConnection(
                username,
                incomingConnection.getIpEndPoint(),
    ...
    private CompletableFuture<IMessageConnection> establishIndirectChild(ConnectToPeerResponse response) {
    ...
    private CompletableFuture<ParentCandidate> getParentCandidateConnectionAsync(
        ...
        diagnostic.debug("Attempting simultaneous direct and indirect parent candidate " + "connections to " + username
                + " (" + ipEndPoint + ")");
        CompletableFuture<IMessageConnection> direct =
                getParentCandidateConnectionDirectAsync(username, ipEndPoint, directCancellation.token());
        CompletableFuture<IMessageConnection> indirect =
        ...
        return firstSuccessful(direct, indirect)
                .thenCompose(winner -> {
                    ...
                    IMessageConnection connection = winner.value();
    ...
    private CompletableFuture<IMessageConnection> getParentCandidateConnectionDirectAsync(
            String username, InetSocketAddress ipEndPoint, CancellationToken cancellationToken) {
    ...
    private CompletableFuture<IMessageConnection> getParentCandidateConnectionIndirectAsync(
    ...
    private CompletableFuture<BranchInformation> waitForParentCandidateConnectionInitializationAsync(
            IMessageConnection connection, CancellationToken cancellationToken) {
    ...
    private record ParentCandidate(IMessageConnection connection, int branchLevel, String branchRoot) {}
    ...
    private static final class LinkedCancellation implements AutoCloseable {
        ...
        private LinkedCancellation(CancellationToken parent) {
            registration = DistributedConnectionManager.token(parent).register(source::cancel);
        }
