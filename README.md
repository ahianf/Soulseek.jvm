...
Searches block until they complete or their cancellation signal fires. Results
are immutable snapshots that can be handed directly to a download request.
...
SearchResult result = slsk.search().run(
        SearchQuery.of("artist album"), CancellationSignal.none());
...
if (!result.isEmpty()) {
    ...
    Download finished = slsk.downloads().await(id, CancellationSignal.none());
...
Thread.startVirtualThread(() ->
        slsk.search().run(
                SearchQuery.of("live recording"), CancellationSignal.none()));
```

Long-running calls accept an explicit `CancellationSignal`; thread interruption
is not the cancellation API because interrupting a virtual thread in socket I/O
can close the socket. State is exposed as immutable snapshots, while sealed
event streams carry deltas. The runtime artifact has no third-party
dependencies.
